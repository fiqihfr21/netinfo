...
@section('content')
  ...
  <div class="col-sm-6 col-sm-offset-3">
    <h1 align="center">Comment</h1><hr>
    <p style="float:left;"><b>{{ $object->name }}</b> - </p>
    <p><small><i>
      @if( $object->created_at->subDays(30) )
        {{ $object->created_at->diffForHumans() }}
      @else
        {{ $object->created_at->format('j M Y , g:ia') }}
      @endif</i></small>
    </p>
    <p class="break-word">{{ $object->description }}</p>
    <form action="comment/{{ $object->id }}" method="post" enctype="multipart/form-data">
      @csrf
      <input type="hidden" value="{{ $object->id }}" name="postid">
      <div class="row">
        <div class="col-md-10">
          <input type="text" name="comment" class="form-control commenttext" placeholder="Write a Comment...">
        </div>
        <div class="col-md-2" style="margin-left:-25px;">
          <button type="type" class="btn btn-primary"><i class="fa fa-comment"></i> Send</button>
        </div>
      </div>
    </form>
    <hr>
  @forelse  ($comments as $comment)
    <div class="">
      <p style="float:left"><b>{{ $object->name }}</b> - </p>
      <p><small><i>
        @if( $comment->created_at->subDays(30) )
          {{ $comment->created_at->diffForHumans() }}
        @else
          {{ $comment->created_at->format('j M Y , g:ia') }}
        @endif</i></small>
      </p>
